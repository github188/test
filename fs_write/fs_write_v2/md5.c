...
static void
md5_process(md_context * ctx, const u8 data[CSUM_CHUNK])
{
	u32 X[16], A, B, C, D;
...
void
md5_update(md_context * ctx, const u8 * input, u32 length)
{
	u32 left, fill;
...
static u8 md5_padding[CSUM_CHUNK] = { 0x80 };

void
md5_result(md_context * ctx, u8 digest[MD5_DIGEST_LEN])
{
	u32 last, padn;
	u32 high, low;
	u8 msglen[8];
...
void
get_md5(u8 * out, const u8 * input, int n)
{
	md_context ctx;
	md5_begin(&ctx);
